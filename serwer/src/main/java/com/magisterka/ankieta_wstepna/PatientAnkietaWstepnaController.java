...
public class PatientAnkietaWstepnaController {
    ...
    public Optional<AnkietaWstepna> getPatientInterview() {
        ...
        return ankietaWstepnaRepository.findByPatientId(patientId)
                .map(entity -> new AnkietaWstepna(entity.getHeight(), entity.getBloodType(), entity.getTrybZycia(), entity.getPrzyjmowaneLeki(), entity.getAlergie()));
    }
    ...
    public AnkietaWstepna putPatientInterview(@NotNull @Valid @Body AnkietaWstepna ankietaWstepna) {
        long patientId = authAttributesProvider.getPatientId();
        AnkietaWstepnaEntity entity = new AnkietaWstepnaEntity(null, patientId, ankietaWstepna.getWzrost(), ankietaWstepna.getGrupaKrwi(), ankietaWstepna.getTrybZycia(), ankietaWstepna.getPrzyjmowaneLeki(), ankietaWstepna.getAlergie());
        return ankietaWstepnaRepository.findByPatientId(patientId)
                .map(existing -> {
                ...
                .map(value -> new AnkietaWstepna(value.getHeight(), value.getBloodType(), value.getTrybZycia(), value.getPrzyjmowaneLeki(), value.getAlergie()))
                .orElseThrow();
    }
}
